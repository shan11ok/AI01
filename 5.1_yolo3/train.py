...
#设置并行线程数和动态分配显存
#原版config = tf.ConfigProto()
config = tf.compat.v1.ConfigProto()
config.gpu_options.allocator_type = 'BFC' #A "Best-fit with coalescing" algorithm, simplified from a version of dlmalloc.
config.gpu_options.per_process_gpu_memory_fraction = 0.7
config.gpu_options.allow_growth = True # 不全部占满显存, 按需分配
#原版set_session(tf.Session(config=config)) 
...
if __name__ == "__main__":
    ...
    # 载入预训练权重，迁移学习的思想，yolo3给出的权重已经很适合目标检测
    ...
    ###把前249层冻结，不进行训练，前面的权重按yolo3给的weight
    freeze_layers = 249
    ...
    # 对train.txt中的图片划分：0.1用于验证，0.9用于训练
    val_split = 0.1
    ...
    # 解冻后训练，注意initial_epoch与上一段训练的epoches一致
    if True:
        model.compile(optimizer=Adam(lr=1e-4), loss={
            'yolo_loss': lambda y_true, y_pred: y_pred})
        #所有层解冻之后，batch_size调小，数据量大容易爆显存
        batch_size = 2
        print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
        model.fit_generator(data_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes),
                steps_per_epoch=max(1, num_train//batch_size),
