...
imp.reload(sys)
fontC = ImageFont.truetype("./font/platech.ttf", 46, 0);
...
def drawRectBox(image, rect, addText):
    ...
    cv2.rectangle(image, (int(rect[0]-1), int(rect[1])-16-46), (int(rect[0] + 115+140), int(rect[1])), (0, 0, 255), -1, cv2.LINE_AA)

    img = Image.fromarray(image)
    draw = ImageDraw.Draw(img)
    #draw.text((int(rect[0]+1), int(rect[1]-16)), addText.decode("utf-8"), (255, 255, 255), font=fontC)
    #draw.text((int(rect[0] + 1), int(rect[1] - 16)), addText, (255, 255, 255), font=fontC)
    draw.text((int(rect[0]+1), int(rect[1]-16-46)), addText, (255, 255, 255), font=fontC)
    imagex = np.array(img)
